...
try {
	...
	if(isset($_POST['msgs']) || isset($_GET['msgs']) ) {
		...
		*/
		...
		$server_time = time();
		writeln_strerr("server_time = " . $server_time);
		if( $result = $sql->query($query) ) {
			$list = array();
			while ($row = $result->fetch_object()){
				writeln_strerr("event_id = " . $row->log_id);
				writeln_strerr("event_updated_str = " . $row->updated);
				writeln_strerr("event_updated = " . strtotime($row->updated));
				$data = array(
					'log_id' => $row->log_id,
